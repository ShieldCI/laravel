...
### Fixed
- `OpcacheAnalyzer` no longer false-positives on Laravel Cloud or Docker — on Cloud, all OPcache configuration sub-checks are suppressed because Laravel Cloud only documents `memory_limit` as configurable (via `ini_set()` in `public/index.php`) and provides no mechanism to change OPcache directives; on Docker, only the three `PHP_INI_SYSTEM` checks (`memory_consumption`, `interned_strings_buffer`, `max_accelerated_files`) are suppressed since the base image controls these settings, while the `PHP_INI_ALL` checks (`validate_timestamps`, `revalidate_freq`) remain active because users control their container builds; two new private methods `isLaravelCloud()` and `isDocker()` are added following the identical pattern already present in `PHPIniAnalyzer`, and both respect the existing `setDeploymentPlatform()` override for unit testing (#182)
- `PHPIniAnalyzer` no longer flags `display_errors`, `display_startup_errors`, `log_errors`, or `ignore_repeated_errors` on Laravel Cloud — these `PHP_INI_ALL` directives were previously still checked on Cloud based on a comment claiming they were actionable via `public/.user.ini`, but that mechanism is undocumented by Laravel Cloud; the only documented customisation is `memory_limit`; these four checks now join `allow_url_fopen`, `allow_url_include`, and `expose_php` (which were already suppressed) in a Cloud-only `unset` block, so the analyzer passes cleanly on a standard Cloud deployment; on Docker, `PHP_INI_ALL` checks remain active as before (#182)
- `CacheHeaderAnalyzer` no longer emits a misleading "missing Cache-Control headers" message on Laravel Cloud — Cloud automatically applies a `Cache-Control: public, max-age=7200` (or similar) header to all cacheable asset responses, so the issue is a short-lived TTL on versioned/hashed assets rather than absent headers; the issue message and result summary now read "short-lived cache headers" on Cloud while the finding itself (and the middleware recommendation) remains, because Cloud explicitly documents that developers can override cache rules by setting their own `Cache-Control` headers, and its deployment-triggered CDN purge means long-lived caching (`max-age=31536000, immutable`) carries no stale-content risk (#182)
- `AnalyzeCommand` and `Reporter` — four pre-existing PHPStan Level 9 errors corrected: `$issue->recommendation ?? ''` simplified to `$issue->recommendation` in `AnalyzeCommand` (the property is a non-nullable `string`); two `isset($issue->severity)` guards removed in `Reporter` (the property is a non-nullable `Severity` enum); `$issue->metadata ?? []` simplified to `$issue->metadata` in `DirectoryWritePermissionsAnalyzerTest` (the property is a non-nullable `array`) (#182)
