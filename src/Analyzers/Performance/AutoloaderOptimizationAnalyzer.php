...
class AutoloaderOptimizationAnalyzer extends AbstractFileAnalyzer
{
    ...
    {
        // Primary check: autoload_static.php exists with actual optimization content
        // This file is generated by Composer 1.2+ when using -o or --optimize-autoloader
        $staticAutoloadPath = $this->buildPath('vendor', 'composer', 'autoload_static.php');

        if (file_exists($staticAutoloadPath)) {
            // Validate that it's a real optimized file, not just an empty stub
            $content = FileParser::readFile($staticAutoloadPath);
        ...
        // Fallback for older Composer versions (pre-1.2):
        // Check if classmap has substantial content from optimization scan
