...
class DebugLogAnalyzerTest extends AnalyzerTestCase
{
    ...
    public function test_detects_global_log_level_when_channel_level_is_null(): void
    {
        // Test case where channel has no specific level, but global LOG_LEVEL is debug
        $analyzer = $this->createAnalyzer([
            'app.env' => 'production',
            'logging.default' => 'single',
            'logging.channels.single.level' => null,
            'logging.level' => 'debug',  // Global LOG_LEVEL
        ]);
    ...
    public function test_channel_specific_level_overrides_global_level(): void
    {
        // Channel-specific level should take precedence over global LOG_LEVEL
        $analyzer = $this->createAnalyzer([
            'app.env' => 'production',
            'logging.default' => 'single',
            'logging.channels.single.level' => 'info',  // Channel-specific
            'logging.level' => 'debug',  // Global LOG_LEVEL (should be ignored)
        ]);

        $result = $analyzer->analyze();

        // Should pass because channel-specific level is 'info', not 'debug'
        $this->assertPassed($result);
    }

    public function test_detects_global_log_level_debug_uppercase(): void
    {
        $analyzer = $this->createAnalyzer([
            'app.env' => 'production',
            'logging.default' => 'single',
            'logging.channels.single.level' => null,
            'logging.level' => 'DEBUG',  // Uppercase global level
        ]);
    ...
    public function test_passes_when_global_level_is_info(): void
    {
        $analyzer = $this->createAnalyzer([
            ...
            'logging.default' => 'single',
            'logging.channels.single.level' => null,
            'logging.level' => 'info',  // Global level is not debug
        ]);
    ...
    public function test_global_level_affects_stack_channels(): void
    {
        // When using stack and channels have no specific level, global level applies
        $analyzer = $this->createAnalyzer([
            'app.env' => 'production',
            'logging.default' => 'stack',
            'logging.channels.stack.channels' => ['single', 'daily'],
            'logging.channels.single.level' => null,
            'logging.channels.daily.level' => null,
            'logging.level' => 'debug',  // Global LOG_LEVEL affects all channels
        ]);
        ...
        $this->assertFailed($result);
        $issues = $result->getIssues();
        // Both channels should be flagged (stack itself + single + daily = 3 issues)
    ...
    public function test_metadata_includes_is_global_level_flag(): void
    {
        // Test channel-specific level (is_global_level should be false)
        $analyzer = $this->createAnalyzer([
            'app.env' => 'production',
            'logging.default' => 'single',
            'logging.channels.single.level' => 'debug',
